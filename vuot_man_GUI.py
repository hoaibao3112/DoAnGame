...
class main2GUI:
    ...
    def addControls(self):
        ...
        self.level_buttons = [
            self.level_buttons1, self.level_buttons2, self.level_buttons3,
            self.level_buttons4, self.level_buttons5, self.level_buttons6,
            self.level_buttons7, self.level_buttons8, self.level_buttons9
            ]

        # Cập nhật màu và trạng thái enabled theo completed_wave
        for i, btn in enumerate(self.level_buttons):
            is_unlocked = i <= self.completed_wave
            btn.color = (0, 200, 0) if is_unlocked else (100, 100, 100)
    ...
    # Cập nhật màu của các nút tương ứng với các level đã mở khóa
        if self.completed_wave >= 0:
            self.level_buttons1.enabled = True
        else:
            self.level_buttons1.enabled = False
        
        if self.completed_wave >= 1:
            self.level_buttons2.enabled = True
        else:
            self.level_buttons2.enabled = False

        if self.completed_wave >= 2:
            self.level_buttons3.enabled = True
        else:
            self.level_buttons3.enabled = False

        if self.completed_wave >= 3:
            self.level_buttons4.enabled = True
        else:
            self.level_buttons4.enabled = False

        if self.completed_wave >= 4:
            self.level_buttons5.enabled = True
        else:
            self.level_buttons5.enabled = False

        if self.completed_wave >= 5:
            self.level_buttons6.enabled = True
        else:
            self.level_buttons6.enabled = False

        if self.completed_wave >= 6:
            self.level_buttons7.enabled = True
        else:
            self.level_buttons7.enabled = False

        if self.completed_wave >= 7:
            self.level_buttons8.enabled = True
        else:
            self.level_buttons8.enabled = False

        if self.completed_wave >= 8:
            self.level_buttons9.enabled = True
        else:
            self.level_buttons9.enabled = False

            # Sau khi cập nhật, vẽ lại các nút
        self.addControls()
